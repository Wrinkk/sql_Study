...
-- DPSVR(길이)
SELECT CONCAT('제 포켓몬', SPACE(3), '이고, 속성은', SPACE(6) '입니다');

-- SUBSTRING(문자열, 시작위치, 길이)
	SELECT 
			  SUBSTRING('열심히  DB공부를 해봅시다.', 5, 4)
			  SUBSTRING ('열심히 DB공부를 해보십시다!', 11)

-- -- SUBSTRING_INDEX(문자열, 시작위치, 길이)
SELECT
		  SUBSTRING_INDEX ('010-4444-7777', '-',2)

		  SUBSTRING_INDEX ('010-4444-7777', '-',-1)
...
-- CELING(숫자)   
SELECT CEILING(123456), FLOOR(1234,.6), ROUND(1234.56);
...
-- TRINCATE(숫자, 정수)
SELECT TRUNCATE(12345, 2), TRUNCATE(12345.12345, -2);
...
SELECT @@GLOBAL.TIME_ZONE; -- 현재 타임존 기준을 확인 하는 조회문
...
-- DATOFWEEK(날짜), MONTHNAME(날짜), DAYOFYEAR(날짜)
...
-- TIME_TO_SEC(시간)
SELECT TIME_TO_SEC(CURTIME()); -- 오늘 흐른시간